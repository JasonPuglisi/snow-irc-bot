	...
	getLastfmPlaying: function(client, network, channel, command, trigger, nickname, target, args, prefix) {
		...
		if (lastfmKey !== undefined) {
			...
			if (input !== undefined) {
				...
				var playingUrl = 'http://ws.audioscrobbler.com/2.0/?method=user.getrecenttracks&user=' + user + '&api_key=' + lastfmKey + '&format=json';
				...
				request(playingUrl, function (error, response, body) {
					if (!error && response.statusCode === 200) {
						...
						if (data.recenttracks !== undefined) {
							if (data.recenttracks.track !== undefined && data.recenttracks.track[0]['@attr']) {
								var track = data.recenttracks.track[0].name;
								var artist = data.recenttracks.track[0].artist['#text'];
								var url = data.recenttracks.track[0].url;

								rpc.emit('call', client, 'privmsg', [target, prefix + user + ' is playing \'' + track + '\' by ' + artist + ' • ' + url]);
							}
